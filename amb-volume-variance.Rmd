...
scheduling_data_raw <- scheduling_data_raw %>%
  ...
  mutate(Appt.Status = case_when(Appt.Status == "Canceled" & Appt.DateYear == as.Date(Appt.Cancel.DateYear) ~ "Same-Day Canceled",
                                 Appt.Status == "Rescheduled" & Appt.DateYear == as.Date(Appt.Cancel.DateYear) ~ "Same-Day Rescheduled",
                                 Appt.Status == "Bump" & Appt.DateYear == as.Date(Appt.Cancel.DateYear) ~ "Same-Day Bump",
                                 TRUE ~ Appt.Status))
...
no_show_specialty <- scheduling_data_raw %>%
  filter(Appt.Status %in% c("Arrived", "No Show", "Same-Day Canceled", "Same-Day Rescheduled", "Same-Day Bump")) %>%
  group_by(Campus, Campus.Specialty, Appt.Year, Appt.MonthYear, New.PT2, Appt.Status) %>%
  summarise(total = n()) %>%
...
# Add Missing ApptStatusFinal
  nms <- c("Campus", "Campus.Specialty", "Appt.Year", "Appt.MonthYear", "New.PT2",
          "Arrived", "No Show", "Same-Day Canceled", "Same-Day Rescheduled", "Same-Day Bump")   # Vector of columns you want in this data.frame
...
no_show_dept <- scheduling_data_raw %>%
  filter(Appt.Status %in% c("Arrived", "No Show", "Same-Day Canceled", "Same-Day Rescheduled", "Same-Day Bump")) %>%
  group_by(Campus, Campus.Specialty, Department, Appt.Year, Appt.MonthYear, New.PT2, Appt.Status) %>%
  summarise(total = n()) %>%
  pivot_wider(names_from = Appt.Status,
              values_from = total,
              values_fill = 0)
...
# Add Missing ApptStatusFinal
  nms <- c("Campus", "Campus.Specialty", "Department", "Appt.Year", "Appt.MonthYear", "New.PT2",
          "Arrived", "No Show", "Same-Day Canceled", "Same-Day Rescheduled", "Same-Day Bump")   # Vector of columns you want in this data.frame
...
no_show_ytd_mshs <- no_show_specialty %>%
  filter(Appt.Year == current_year) %>%
  group_by(Campus, Campus.Specialty, New.PT2) %>%
  summarise(Arrived = sum(Arrived),
            `NoShow` = sum(`No Show`),
            `SameDayCanceled` = sum(`Same-Day Canceled`),
            `SameDayRescheduled` = sum(`Same-Day Rescheduled`),
            `SameDayBump` = sum(`Same-Day Bump`),
            Total = sum(Total)) %>%
...
no_show_ytd_site_function <- function(site){
  ...
  no_show_ytd_dept <- no_show_dept %>%
    filter(Campus == site) %>%
    group_by(Campus, Campus.Specialty, Department, New.PT2) %>%
    summarise(Arrived = sum(Arrived),
              `NoShow` = sum(`No Show`),
              `SameDayCanceled` = sum(`Same-Day Canceled`),
              `SameDayRescheduled` = sum(`Same-Day Rescheduled`),
...
no_show_trend_site_function <- function(site){
  ...
  no_show_trend_site <- no_show_dept %>%
    group_by(Campus, Campus.Specialty, Appt.MonthYear, New.PT2) %>%
    summarise(Arrived = sum(Arrived),
              `NoShow` = sum(`No Show`),
              `SameDayCanceled` = sum(`Same-Day Canceled`),
              `SameDayRescheduled` = sum(`Same-Day Rescheduled`),
...
map(unique(no_show_trend_site_output$Campus.Specialty), function(x){
highchart() %>%
  hc_yAxis_multiples(
    # list(title = list(text = "Avg Volume per Day"), lineWidth = 3),
    list(title = list(text = "No Show Rate"), lineWidth = 3, 
  ...
  hc_xAxis(title = list(text = ""),
           categories = (no_show_trend_site_output %>% filter(Campus.Specialty == x))$Appt.MonthYear,
           type = "datetime",
           labels = list(format = '{value:%Y-%m}')
           # dateTimeLabelFormats = list(month = '%Y-%m')
           ) %>%
  ...
  #               dataLabels = list(enabled = TRUE, format='{point.y}')) %>%
  hc_add_series(name   = "Established",
                data   = (no_show_trend_site_output %>% filter(Campus.Specialty == x))$noShow_rate_Established,
                type = 'line', 
                # color = '#a5a7a5'
                marker = list(enabled= TRUE),
                color='#212070',
  ...
  hc_add_series(name   = "New",
                data   = (no_show_trend_site_output %>% filter(Campus.Specialty == x))$noShow_rate_New,
                type = 'line', 
                # color = '#a5a7a5'
                marker = list(enabled= TRUE),
                color='#d80b8c',
  ...
  hc_add_series(name   = "All",
                data   = (no_show_trend_site_output %>% filter(Campus.Specialty == x))$noShow_rate,
               type = 'line', 
                # color = '#a5a7a5'
                marker = list(enabled= TRUE),
                color='#00aeef',
